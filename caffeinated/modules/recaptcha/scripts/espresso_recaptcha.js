...
jQuery(document).ready(function ($) {
    ...
    /**
     * @namespace {object} EE_RECAPTCHA
     * @type {{
		 *     recaptcha_div: object,
		 *     not_a_robot: boolean
	 * }}
     * @namespace {object} eei18n
     * @type {{
    ...
    EE_RECAPTCHA = {
        ...
        initialize: function () {
            ...
            if (EE_RECAPTCHA.recaptcha_div === 'undefined' || EE_RECAPTCHA.recaptcha_div.length === 0) {
                // if WP_DEBUG is on, then display an error
                if (eei18n.wp_debug) {
                    var msg = SPCO.tag_message_for_debugging(
                        'EE_RECAPTCHA.initialize() error',
                        eei18n.no_recaptcha_error
                    );
                    SPCO.scroll_to_top_and_display_messages(
                        SPCO.main_container,
                        SPCO.generate_message_object('', msg, ''),
                        true
                    );
                }
                return;
        ...
        process_SPCO_response: function (SPCO_response) {
            ...
            if (typeof SPCO_response.recaptcha_passed !== 'undefined') {
                ...
                if (SPCO_response.bypass_recaptcha) {
                    ...
                    EE_RECAPTCHA.display_error(
                        SPCO.tag_message_for_debugging('EE_RECAPTCHA.passed() error', eei18n.recaptcha_fail)
                    );
                }
            }
        },
...
var espresso_recaptcha_verification = function (response) {
    jQuery(document).ready(function ($) {
        if (
            ...
            && response !== ''
            && typeof SPCO.additional_post_data !== 'undefined'
            && SPCO.additional_post_data.indexOf('g-recaptcha-response') === -1
        ) {
