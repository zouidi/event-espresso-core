...
class EEM_CPT_Base_Test extends EE_UnitTestCase{
	 ...
	 * that are trashed, BUT DO NOT find CPT items of the wrong post_type
	 * (which might have an entry in the meta table due to some hackery)
	 *
	 * @group 9179
	 */
	public function test_get_minimum_where_conditions_during_query() {
		...
		$this->factory->post->create_and_get( array( 'post_type' => 'monkey' ) );
		...
		$this->assertEquals( $existing_post_count + 3, EEM_Event::instance()->count( array( 'default_where_conditions' => 'none' ) ) );
	}
}


// Location: testcases/core/db_models/EEM_CPT_Base_Test.php