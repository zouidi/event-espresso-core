...
class EEM_Answer_Test extends EE_UnitTestCase{
	...
	public function test_get_answer_value_to_question(){
		...
		$this->assertEquals(
			$a->value(),
			EEM_Answer::instance()->get_answer_value_to_question( $r, $a->question_ID(), false )
		);
		$this->assertEquals(
			$r->attendee()->fname(),
			EEM_Answer::instance()->get_answer_value_to_question(
				$r,
				EEM_Question::instance()->get_Question_ID_from_system_string(
	...
	public function test_set_ANS_value__with_unsafe_html(){
		$a = $this->new_model_obj_with_dependencies(
			'Answer',
			array(
