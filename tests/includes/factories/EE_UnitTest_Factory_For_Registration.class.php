	...
	protected function _set_default_properties_and_relations( $called_class ) {
		...
		if ( empty( $this->_default_properties ) ) {
			$this->_default_properties = array(
				...
				'REG_paid'        => 0,
				'REG_session'     => uniqid(),
				'REG_code'        => new WP_UnitTest_Generator_Sequence( "1-1-%d-" . substr( uniqid(), 0, 4 ) ),
				'REG_url_link'    => new WP_UnitTest_Generator_Sequence( '%s' . md5( 'ticket' . microtime() ) ),
				'REG_count'       => new WP_UnitTest_Generator_Sequence( '%s', 1 ),
				'REG_group_size'  => new WP_UnitTest_Generator_Sequence( '%s', 1 ),
