...
class EE_Data_Migration_Manager{
	 ...
	 * Also has a side-effect of recording which was the last ran,
	 * and which was the last ran which hasn't finished yet
	 *
	...
	public function parse_dms_classname($classname){
		$matches = array();
		preg_match( '~EE_DMS_(.*)_(\d*)_(\d*)_(\d*)~', $classname, $matches );
		...
		return array(
			'slug' => $matches[1],
			'major_version' => (int)$matches[2],
			'minor_version' => (int)$matches[3],
			'micro_version' => (int)$matches[4]
		);
	}
		...
		do{
			...
			foreach($script_class_and_filepaths_available as $classname => $filepath){
				...
				if(
					! $scripts_ran
					|| ! isset(
						$scripts_ran[$script_converts_plugin_slug],
					...
					$scripts_ran[$script_converts_plugin_slug][$script_converts_to_version] instanceof EE_Data_Migration_Script_Base
				){
					//this script has been ran, or at least started
					$script = $scripts_ran[$script_converts_plugin_slug][$script_converts_to_version];
					if (
						$script instanceof EE_Data_Migration_Script_Base
						&& $script->get_status() !== self::status_completed
					) {
							//this script is already underway... keep going with it
							$script_classes_that_should_run_per_iteration[ $iteration ][ $script->priority(
							) ][] = $script;
							$migrates_to_version = $script->migrates_to_version();
							$next_database_state_to_consider[ $migrates_to_version['slug'] ] = $migrates_to_version['version'];
							unset( $script_class_and_filepaths_available[ $classname ] );
...
//		//start output buffer just to make sure we don't mess up the json
	...
	public function update_current_database_state_to($slug_and_version = null){
		...
		$current_database_state = (array)get_option( self::current_database_state, array() );
		...
		if( ! $this->_data_migration_class_to_filepath_map){
			...
			foreach($this->get_data_migration_script_folders() as $folder_path){
				...
				foreach($files as $file){
					...
					if( $slug !== 'Core' && ! EE_Registry::instance()->get_addon_by_name( $slug ) ) {
	...
	public function add_error_to_migrations_ran($error_message){
		...
		global $wpdb;
		$last_migration_script_option = (array)$wpdb->get_row(
		...
		if(
			...
			&& isset( $last_ran_migration_script_properties['class'], $last_ran_migration_script_properties['_status'] )
		){
	...
	//			echo "saving migration script to $version_string<br>";
				$option_name = self::data_migration_script_option_prefix.$plugin_slug.'.'.$version_string;
	...
	public function _instantiate_script_from_properties_array($properties_array){
		if ( ! isset( $properties_array['class'] ) ) {
			throw new EE_Error(
				sprintf(
					__(
						"Properties array  has no 'class' properties. Here's what it has: %s",
						'event_espresso'
					),
		...
		if ( ! class_exists( $class_name ) ) {
			throw new EE_Error(
				sprintf( __( 'There is no migration script named %s', 'event_espresso' ), $class_name )
			);
		}
...
//				$details = $this->parse_dms_classname($classname);
				...
				if(
					...
					&& version_compare( $champion_migrates_to[ 'version' ], $contender_migrates_to[ 'version' ], '<' )
				){
	...
	public function get_migration_ran( $version, $plugin_slug = 'Core' ) {
		...
		if( isset( $migrations_ran[ $plugin_slug ], $migrations_ran[ $plugin_slug ][ $version ] ) ){
