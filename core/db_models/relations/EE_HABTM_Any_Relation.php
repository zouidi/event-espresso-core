...
class EE_HABTM_Any_Relation extends EE_HABTM_Relation{
	...
	/**
	 * Object representing the relationship between two models.
	 * HasAndBelongsToMany relations always use a join-table (and an ee joining-model.)
	 * This knows how to join the models, get related models across the relation,
	 * and add-and-remove the relationships.
	 *
	 * @param boolean $block_deletes for this type of relation, we block by default for now.
	 *                               if there are related models across this relation,
	 *                               block (prevent and add an error) the deletion of this model
	 * @param string $blocking_delete_error_message a customized error message on blocking deletes
	...
	/**
	 * Gets the SQL string for joining the main model's table containing the pk to the join table.
	 * 	Eg "LEFT JOIN real_join_table AS join_table_alias ON this_table_alias.pk = join_table_alias.fk_to_this_table"
	 *
	...
	function get_join_to_intermediate_model_statement($model_relation_chain){
		...
		$SQL .= $this->get_join_model()->_construct_internal_join_to_table_with_alias( $join_table_alias );
		return $SQL;
	}
	...
	/**
	 * Gets the SQL string for joining the join table to the other model's pk's table.
	 * Eg "LEFT JOIN real_other_table AS other_table_alias ON join_table_alias.fk_to_other_table = other_table_alias.pk"
	 * If you want to join between modelA -> joinModelAB -> modelB
	 * (eg, Event -> Event_Question_Group -> Question_Group),
	...
	function get_join_statement($model_relation_chain){
		if( $this->_model_relation_chain_to_join_model === NULL ){
			throw new EE_Error(
				sprintf(
					__( 'When using EE_HABTM_Relation to create a join, you must call get_join_to_intermediate_model_statement BEFORE get_join_statement', 'event_espresso' )
				)
			);
		}
		$join_table_fk_field_to_this_table = $this->get_join_table_fk_field_to(
		...
		$SQL .= $this->get_other_model()->_construct_internal_join_to_table_with_alias($other_table_alias);
		return $SQL;
	}
	...
	/**
	 * Ensures there is an entry in the join table between these two models.
	 * Feel free to do this manually if you like.
	 *
	 * @param EE_Base_Class /int $this_obj_or_id
	 * @param EE_Base_Class/int $other_obj_or_id
	 * @param array $extra_join_model_fields_n_values col=>val pairs that are used as extra conditions
	 *                                                for checking existing values and for setting new rows
	 *                                                if no exact matches.
	 ...
	 function add_relation_to( $this_obj_or_id, $other_obj_or_id, $extra_join_model_fields_n_values = array() ){
		...
		//check if such a relationship already exists
		 ...
		 if ( ! empty( $extra_join_model_fields_n_values ) ) {
			 // make sure we strip any of the join model names from the $where_query
			 // cause we don't need that in here
			 // (why? because client code may have used the same conditionals
			 // for get_all_related which DOES need the join model name)
		...
		$existing_entry_in_join_table = $this->get_join_model()->get_one($query_params);
		 ...
		 // again, if you want more sophisticated logic or insertions
		 // (handling more columns than just 2 foreign keys to the other tables),
		 // use the joining model directly!
	 ...
	 * @param EE_Base_Class | int 	$this_obj_or_id
	 * @param EE_Base_Class | int 	$other_obj_or_id
	 * @param array 				$where_query  	col=>val pairs that are used as extra conditions
	 *                                              for checking existing values and for removing existing rows
