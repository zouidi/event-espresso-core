      ...
      switch ($meta['bits']) {
        ...
        case 16:
          ...
          if (empty($meta['rMask']) || $meta['rMask'] != 0xf800)
            $color[1] = (($color[1] & 0x7c00) >> 7) * 65536 + (($color[1] & 0x03e0) >> 2) * 256 + (($color[1] & 0x001f) << 3);           else 
            $color[1] = (($color[1] & 0xf800) >> 8) * 65536 + (($color[1] & 0x07e0) >> 3) * 256 + (($color[1] & 0x001f) << 3);           break;
        case 8:
...
function record_warnings($errno, $errstr, $errfile, $errline) {
  ...
  if ( !($errno & (E_WARNING | E_NOTICE | E_USER_NOTICE | E_USER_WARNING )) )     throw new DOMPDF_Exception($errstr . " $errno");
