 ...
 *
 * This contains the logic for setting up the Event Maintenance related admin pages.
...
class Maintenance_Admin_Page extends EE_Admin_Page {
	...
	/**
	 * wraps maintenance() to fix naming;
	 *
	 * @throws \EE_Error
	 */
	...
	public function maintenance(){
		...
		try{
			...
			if( $mm === EE_Maintenance_Mode::level_2_complete_maintenance ) {
				if( ! $placed_in_mm ){
					...
					EE_Error::add_attention(
						__( 'Site taken out of maintenance mode because no data migration scripts are required', 'event_espresso' )
					);
				...
				$show_backup_db_text = ! isset( $this->_req_data['continue_migration'] ) ? true : false;
		...
		if($exception_thrown ||
				...
				$this->_template_args = array_merge(
					...
					array(
						'current_db_state' => sprintf(
							...
							isset( $current_db_state[ $plugin_slug ] )
			...
			wp_localize_script(
				...
				array(
					...
					'click_next_when_ready'            => __(
						"The current Migration has ended. Click 'next' when ready to proceed",
						"event_espresso"
					),
		...
		$this->_template_args['maintenance_mode_on'] = $mm;
	...
	public function _change_maintenance_level(){
		$new_level = (int)$this->_req_data['maintenance_mode_level'];
...
//					'content-type:text/html charset=UTF-8'
	...
	public function _confirm_migration_crash_report_sent(){
		...
		$this->_template_args['success'] = (int)$this->_req_data['success'] === 1 ? true : false;
		$this->_template_args['most_recent_migration'] = $most_recent_migration;
	...
	public function _reset_db( $nuke_old_ee4_data = TRUE ){
		...
		EE_Registry::instance()->CFG = EE_Config::reset( TRUE );
		EE_Activation_Manager::instance()->initialize_db_if_no_migrations_required( TRUE );
		EE_Activation_Manager::instance()->redirect_to_about_ee();
	}
